...
class CountHistoryRepository {
  ...
  Future<List<CountHistory>> getTodayCountHistory({String? tasbeehId}) async {
    ...
    return getCountHistoryByDateRange(
      startOfDay,
      endOfDay,
      tasbeehId: tasbeehId,
    );
  ...
  Future<Map<String, dynamic>> getCountStatistics({
    ...
    // Calculate actual sessions: only count completed rounds (count > 1)
    // Individual increments (count = 1) are not sessions, only completed rounds are
    String sessionWhereClause = whereClause.isEmpty
        ? 'WHERE count > 1'
        : '$whereClause AND count > 1';

    final sessionResult = await db.rawQuery('''
      SELECT COUNT(*) as total_sessions
      FROM count_history $sessionWhereClause
    ''', whereArgs);

    final countStats = countResult.first;
    final sessionStats = sessionResult.first;

    return {
      'totalSessions': sessionStats['total_sessions'] as int,
      'totalCount': countStats['total_count'] as int? ?? 0,
      'averageCount': (countStats['average_count'] as double?)?.round() ?? 0,
      'minCount': countStats['min_count'] as int? ?? 0,
      'maxCount': countStats['max_count'] as int? ?? 0,
    };
  }
}
